...
4. **출시 전 정리** — 랜딩 가짜 지표·플레이스홀더 제거, 약관·개인정보, 실 가격 확정
...
- **결제** (2026-08-13에 토스페이먼츠로 대부분 구현 — 위 "결제·멤버십" 섹션 참고)
    - 아직 **테스트 키** — 실 가맹심사·키 전환 전
    - 실 가격 미정 (현재 100~400원대 placeholder)
    - 관리자 콘솔에 결제/구독 내역 조회·환불 UI 없음(`PaymentTransaction`은 DB에만 존재)
    - 영수증 발급 흐름 없음
- **관측성**
